...
import { ModifierPool } from './Modifiers';
...
export class AutoTest extends Component {
    ...
    private testEnemySystem(): void {
        ...
        // 测试第1关敌人
        this.test('第1关敌人血量为50', () => {
            const enemy = getEnemyInfo(1);
            console.log(`  第1关: ${enemy.name}, ${enemy.hp}血`);
            return enemy.hp === 50;
        });
        
        // 测试第5关敌人
        this.test('第5关敌人血量为130', () => {
            const enemy = getEnemyInfo(5);
            console.log(`  第5关: ${enemy.name}, ${enemy.hp}血`);
            return enemy.hp === 130;
        });
        
        // 测试第6关敌人（强敌）
        this.test('第6关敌人血量为155', () => {
            const enemy = getEnemyInfo(6);
            console.log(`  第6关: ${enemy.name}, ${enemy.hp}血`);
            return enemy.hp === 155;
        });
        
        // 测试第11关敌人（精英）
        this.test('第11关敌人血量为280', () => {
            const enemy = getEnemyInfo(11);
            console.log(`  第11关: ${enemy.name}, ${enemy.hp}血`);
            return enemy.hp === 280;
        });
        ...
        this.test('血量增长平滑（无跳跃）', () => {
            ...
            console.log(`  最大增长: ${maxGrowth}`);
            return maxGrowth <= 25; // 增长不超过25
        });
    }
    ...
    private testModifierSystem(): void {
        ...
        // 测试词条池
        this.test('词条池包含17个词条', () => {
            console.log(`  词条总数: ${ModifierPool.length}`);
            return ModifierPool.length === 17;
        });
        
        // 测试稀有度分布
        this.test('Common词条5个', () => {
    ...
    private testDataManager(): void {
        ...
        this.test('清空数据', () => {
            ...
            const stage = DataManager.getInstance().getHighestStage();
            const score = DataManager.getInstance().getHighestScore();
            console.log(`  清空后: 关卡${stage}, 分数${score}`);
