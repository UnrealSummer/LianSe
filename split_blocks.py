"""
分割和调整方块图片
"""

from PIL import Image
import os

def split_and_resize_blocks(input_path, output_dir, target_size=128):
    """
    分割6个方块并调整尺寸
    """
    # 打开图片
    img = Image.open(input_path)
    width, height = img.size
    
    print(f"原始图片尺寸: {width}x{height}")
    
    # 计算每个方块的尺寸（2x3布局）
    block_width = width // 2
    block_height = height // 3
    
    print(f"单个方块尺寸: {block_width}x{block_height}")
    
    # 方块位置和名称
    blocks = [
        (0, 0, "red"),           # 左上
        (1, 0, "orange"),        # 右上
        (0, 1, "yellow"),        # 左中
        (1, 1, "green"),         # 右中
        (0, 2, "blue"),          # 左下
        (1, 2, "purple"),        # 右下
    ]
    
    # 创建输出目录
    os.makedirs(output_dir, exist_ok=True)
    
    print("\n开始分割和调整尺寸...")
    
    for col, row, name in blocks:
        # 计算裁剪区域
        left = col * block_width
        top = row * block_height
        right = left + block_width
        bottom = top + block_height
        
        # 裁剪方块
        block = img.crop((left, top, right, bottom))
        
        # 调整尺寸到128x128
        block_resized = block.resize((target_size, target_size), Image.Resampling.LANCZOS)
        
        # 保存
        output_path = os.path.join(output_dir, f"{name}.png")
        block_resized.save(output_path, "PNG")
        
        print(f"OK {name}.png ({target_size}x{target_size})")
    
    print(f"\n完成！所有方块已保存到: {output_dir}")

if __name__ == "__main__":
    # 输入文件
    input_file = "C:/Users/yu.zhang01/.clawdbot/media/inbound/68446cae-4bd1-49dd-a539-fb8ee9f2ef05.png"
    
    # 输出目录
    output_dir = "E:/Project/LianSe/LSProject/assets/textures/blocks"
    
    # 分割和调整
    split_and_resize_blocks(input_file, output_dir, target_size=128)
