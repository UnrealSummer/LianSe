...
def recrop_button_center():
    ...
    print("="*60)
    print("[Re-cropping Button - Center Position]")
    print("="*60)
    ...
    # 取正中间：y=45%（在40%和50%之间）
    x = int(width * 0.15)
    y = int(height * 0.45)  # 中间位置
    w = int(width * 0.7)
    h = int(height * 0.2)
    ...
    # 保存裁剪版本检查
    check_path = os.path.join(output_dir, "button_base_cropped_v3.png")
    ...
    for target_w, target_h, size_name in target_sizes:
        ...
        output_png = os.path.join(output_dir, f"button_base_{size_name}_v3.png")
        resized.save(output_png, 'PNG', quality=95)
        size_kb = os.path.getsize(output_png) / 1024
        print(f"  Saved: button_base_{size_name}_v3.png ({target_w}x{target_h}, {size_kb:.2f} KB)")
    
    print("\n" + "="*60)
    print("[Complete!]")
    print("="*60)
    print(f"\n请检查 button_base_cropped_v3.png")
    print("这次应该是完整的按钮了")
    print("="*60)

if __name__ == "__main__":
    recrop_button_center()
